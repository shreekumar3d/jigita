...
def compute_corner(p1, p2, p3):  # (p2,p1) and (p2,p3)
    ...
    tangent1 = [vec1[0] * cos_t - vec1[1] * sin_t, vec1[0] * sin_t + vec1[1] * cos_t]
...
def extract_corners_2D(poly_points, angular_thresh=10.0):
    ...
    for start, end in zip(corner_idx, corner_idx[1:]):
        corner_segments.append(
            [
                copy.copy(xy[start : end + 1]),
                [tl1[tangent_idx], tl2[tangent_idx]],
                [tl1[tangent_idx + 1], tl2[tangent_idx + 1]],
            ]
        )
